 ...
 */
public class App22_persitent_Hibernate_genId {
    ...
    private static Timekeeper persist_Transient(Session session, Employee emp) {
        ...
        session.flush(); //đã gửi tới SQL server, nhưng ko apply phải chờ session.commit()
 
        String timekeeperId = tk1.getTimekeeperId(); //mã này do Hibernate tạo ra
